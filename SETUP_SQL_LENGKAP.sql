...
-- ═══ Pembetulan Padam Voucher (FK CASCADE) + Had Maksima Belanja ═══
...
-- ═══ Simpan state Hebahan WhatsApp di cloud (sambung tugasan dari peranti kedua) ═══
...
DROP POLICY IF EXISTS "pemilik urus wa_hebahan_state" ON wa_hebahan_state;
CREATE POLICY "pemilik urus wa_hebahan_state" ON wa_hebahan_state FOR ALL USING (is_pemilik()) WITH CHECK (is_pemilik());

-- ═══ validasi_baucar() kini pulangkan juga minima/maksima belanja, tarikh luput
-- & had guna supaya index.html boleh papar modal "Terma & Syarat Voucher" ═══
...
-- ═══ Voucher "Percuma Penghantaran" (Free Shipping) ═══
...
-- ═══ QR Bayaran + Bukti Resit Wajib di Tab Hantar (resit_bukti_url) ═══
