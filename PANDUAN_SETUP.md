...
> 110. `SQL_TAMBAHAN_137.sql` — **Baiki baris "🔻 Pelarasan Kos Dipulangkan" hilang bila baucar harian dijana semula** (susulan #106, ditemui semasa sahkan PV-2026-0103) — `cipta_baucar_harian()` cuma rekod kunci `butiran.pelarasan` jika jumpa pelarasan `belum_diselaraskan` PADA PANGGILAN NI sahaja; kalau fungsi dipanggil > 1x utk hari/pekerja sama (cth Thumb Out disusuli jana semula) sblm diluluskan, panggilan KEDUA jumpa pelarasan dah `sudah_diselaraskan` (drpd panggilan pertama) → timpa `butiran` TANPA kunci pelarasan, walaupun upah kekal betul ditolak (duit tak salah, cuma jejak audit hilang drpd resit). Baiki: turut kira pelarasan yg SUDAH terikat dgn `baucar_id` draf tsb drpd panggilan sebelum, gabung dgn pelarasan baharu — `butiran.pelarasan` kekal betul tak kira berapa kali dijana semula. Diuji live (BEGIN/ROLLBACK, panggil 2x berturut) + backfill PV-2026-0103 yg terjejas (jumlah tak berubah, cuma tambah kunci pelarasan yg hilang).
> 111. **Baiki kos minyak servis Tukar Barang Rosak/Expired tak dikira langsung** — RPC `tukar_stok_expired_kedai()` (ambil balik barang rosak/expired, gantikan barang baru) HANYA kemaskini `stok_pekerja` + `pelupusan_stok` — TIDAK cipta rekod `transaksi` langsung. `kiraJarakKehadiranJulat()` (kira km/minyak) pula HANYA bina laluan drpd rekod `transaksi` dlm sesi — jadi perjalanan sebenar pekerja ke kedai SEMATA-MATA utk servis tukar (tiada jualan lain hari tu) langsung RM0 minyak walaupun betul-betul memandu ke sana; kalaupun ada jualan LAIN hari sama (minyak hari tu layak dituntut), lokasi kedai servis-sahaja tetap TAK termasuk dlm laluan yg dikira. Baiki: turut ambil lokasi kedai drpd `pelupusan_stok` (jenis `tukar_ambil`/`tukar_beri`, dedielt ikut no. resit — 1 kunjungan jana byk baris item) sbg destinasi sah dlm laluan, taraf sama dgn transaksi jualan. Disahkan kesan sebenar guna data produksi (Aremier, 2 Ogos 2026) — kunjungan tukar-expired ke JHF Food Mart yg sblm ni "senyap" tingkatkan laluan hari tu drpd 41.2km → 168.4km (+RM63.62 minyak). **Nota**: fix ni terpakai utk laporan/baucar BAHARU sahaja (dikira "live" setiap kali dijana) — TIDAK ubah baucar yg dah dijana/dibayar sebelum ini (snapshot kekal). Nota tambahan: jarak besar dlm kes ujian ni turut cadangkan pin JHF Food Mart mungkin tersasar — semak guna "🔴🟡🟢 Indikator Sah Lokasi Kedai" (#109).
> 112. **Baiki "SEBAB" tersalah papar pada resit Tukar Barang Expired** (ditemui semasa semakan susulan #111) — `tukar_stok_expired_kedai()` cipta baris "ambil balik" (sebab SEBENAR, cth "rosak") DAN baris "gantian" (sebab TETAP "gantian_tukar") dlm SATU transaksi RPC — Postgres `now()` BEKU/sama sepanjang satu transaksi, jadi `created_at` KEDUA-DUA jenis baris SAMA 100% (disahkan langsung dlm DB). `renderResitPelupusan()` guna `data[0].sebab` (baris PERTAMA lepas `ORDER BY created_at`) utk papar "SEBAB" — tapi bila timestamp seri, Postgres TAK jamin susunan baris pulangan, jadi resit kadang papar "🔄 Gantian (Tukar Expired)" bukan sebab sebenar (Rosak/Expired/dll) yg pekerja pilih. Baiki: ambil sebab drpd baris "diambil" (tukar_ambil, sentiasa sebab sebenar) secara eksplisit, bukan baris pertama sembarangan. Diuji simulasi kedua-dua susunan (asal & terbalik) — sentiasa papar sebab betul selepas fix.
> 112. **🚨 Baiki kelulusan "Minta Padam Transaksi" (pekerja) langkau logik pulih stok/pelarasan kos** — pekerja tiada kebenaran padam transaksi terus, jadi tekan 🗑️ "Minta pemilik padam" (`mintaPadam()`) hantar permohonan ke jadual `permohonan_padam`. Bila pemilik luluskan (`putuskanPermohonanPadam()`), aliran GENERIK sedia ada buat `sb.from(jadual).delete()` TERUS pada jadual `transaksi` — ni LANGKAU sepenuhnya RPC `padam_transaksi_kedai()` (lihat #106/#107) yg sepatutnya pulangkan stok ke bawaan pekerja, tolak hutang kedai, DAN cipta rekod `pelarasan_kos_pekerja` kalau baucar harian hari tu dah diluluskan/dibayar. Kesan sebenar: kalau pekerja mohon padam transaksi (cth kedai pulangkan/batal pesanan) & pemilik luluskan lepas baucar dah dibayar, stok TAK pulih & kos upah/minyak yg dah terlanjur dibayar TAK direkod utk ditolak drpd baucar seterusnya — sama masalah yg #106 baiki utk laluan padam LANGSUNG pemilik, tapi laluan KEDUA (approval permohonan) ni tercicir drpd fix tu. Baiki: `putuskanPermohonanPadam()` kini kesan jenis `'transaksi'` secara khas & panggil `padam_transaksi_kedai()` RPC (dgn `kiraMinyakPulihTransaksi()` sama spt laluan padam langsung) — jenis lain (kedai/stok/baucar_bayaran/dll) kekal guna delete() generik spt asal.
> 113. `SQL_TAMBAHAN_138.sql` — **Resit Pelupusan/Tukar Expired: paparan lengkap per-kedai + fungsi Padam pulih stok bawaan** (susulan #111/#112) — 2 isu ditemui semasa semakan: (a) `renderSejarah(kedaiId)` SENGAJA skip ambil `pelupusan_stok` bila kedai tertentu ditapis (`if (!kedaiId)`), dan `renderTransaksiKedaiList()` (Tab Kedai→Transaksi) langsung tak baca jadual tu — servis tukar-expired "hilang" drpd sejarah kedai berkenaan walaupun aktiviti sah & kini turut dikira dlm km/minyak; (b) TIADA cara padam resit pelupusan/tukar-expired yg tersalah rekod — pemilik terpaksa betulkan manual di database, DAN kalaupun ada laluan generik (spt isu #112), ia akan padam 1 BARIS sahaja (baki baris lain 1 resit sama tercicir) tanpa pulih stok bawaan pekerja langsung. Baiki: (1) `renderSejarah()` kini tapis `pelupusan_stok` ikut `kedai_id` bila kedai dipilih (bukan skip terus); (2) `renderTransaksiKedaiList()` turut sertakan servis tukar-expired (jenis tukar_ambil/tukar_beri) dlm senarai, guna kad kongsi baharu `kadPelupusanHTML()` (elak drift papar antara 2 tempat); (3) RPC baharu `padam_resit_pelupusan(p_resit)` — padam SEMUA baris 1 resit sekaligus & PULIHKAN stok bawaan pekerja bagi baris yg asalnya tolak stok (pelupusan biasa + gantian/`tukar_beri`) — `tukar_ambil` (barang drpd KEDAI, bukan bag pekerja, lihat nota `submitTukarExpired`) TIADA apa dipulihkan sebab tak pernah ubah stok bawaan pun; (4) ikon padam (`ikonPadam`) ditambah pada setiap kad — pekerja laluan `mintaPadam()` (perlu kelulusan pemilik dahulu), dan `putuskanPermohonanPadam()` turut dikemaskini kesan jenis `pelupusan_stok` secara khas (elak isu SAMA yg #112 baiki utk transaksi — laluan generik `delete()` akan langkau pulihan stok). Diuji live (BEGIN/ROLLBACK, resit sebenar R-260802-237) — padam resit pulihkan tepat 3 unit (gantian) ke stok bawaan Aremier, 17 unit (ambil balik, drpd kedai) betul TIDAK disentuh.
> 114. **📊 Pecahan Hutang Kedai (Terima Bayaran Hutang)** — hutang 1 kedai boleh terdiri campuran jualan terus & consignment yg dah disahkan terjual, TIADA cara pemilik/pekerja nampak pecahan tu semasa nak terima bayaran (cuma papar 1 angka total) — punca kekeliruan sebenar semasa siasat Serambi Bonda (Sep 2026): hutang RM24 nampak "satu jumlah" walhal sebenarnya gabungan 1 transaksi consignment lama, dan RM14.40 lain (consignment produk berbeza, 12 drpd 20 unit terjual, 8 dipulangkan) yg sepatutnya SUDAH selesai berasingan tapi tersilap rekod sbg jualan terus baharu. Tambah panel `pecahanHutangKedai()` — bila kedai dipilih dlm "Terima Bayaran Hutang", papar terus: (a) Hutang Jualan Terus vs Hutang Consignment (dah disahkan) berasingan, tiap dgn bilangan transaksi; (b) jumlah unit + nilai stok consignment yg PERNAH dipulangkan sepanjang masa kedai tu (bukan hutang duit — bukti kenapa jumlah hutang lebih rendah drpd jumlah asal dihantar). Disahkan padan data sebenar Serambi Bonda selepas rekod dibetulkan terus dlm database (hutang consignment RM24 · 1 transaksi, stok dipulangkan 8 unit · nilai RM9.60).
> 115. `SQL_TAMBAHAN_139.sql` — **🔄 Tukar Produk Transaksi (pemilik sahaja)** — pekerja kadang tersalah pilih produk semasa rekod penghantaran/consignment (cth produk A dihantar fizikal tapi produk B direkod dlm app); sebelum ni TIADA cara betulkan selain padam transaksi PENUH & minta pekerja hantar semula (menyusahkan, hilang jejak audit asal). RPC baharu `tukar_produk_transaksi(p_transaksi_id, p_stok_id_lama, p_stok_id_baru)` tukar SATU baris produk (kekalkan kuantiti) & urus SEMUA kesan automatik: pulangkan stok bawaan produk lama + tolak produk baharu (rollback kalau stok baharu tak cukup), kemaskini rujukan stokId di `items`/`items_terjual`/`items_pulang` (consignment separa/penuh disahkan turut betul), kira semula jumlah/jumlah_asal (kekalkan kadar diskaun asal) & laraskan `kedai.hutang` ikut beza jumlah. Butang "🔄 Tukar Produk" (pemilik SAHAJA) ditambah pada setiap kad transaksi (Tab Kedai→Transaksi), buka modal pilih produk asal→produk gantian. Diuji live (BEGIN/ROLLBACK, T5091574 S003→S001) — stok bawaan, rujukan items_terjual & jumlah (RM24→RM26 ikut harga produk baharu) semua betul terlaras.
> 116. **🔍 Carian Transaksi Kedai** — senarai Tab Kedai→Transaksi boleh jadi panjang merentasi bulan/pekerja, susah cari 1 transaksi tertentu (cth nak guna "Tukar Produk" #115) cuma dgn tapisan bulan/status sedia ada. Tambah kotak carian bebas — padan nama kedai/pembeli peribadi, nama produk dlm transaksi, no. rujukan/resit, atau nota, hasil terus tapis live semasa menaip (`oninput`). Turut terpakai kpd kad servis Tukar Barang Rosak/Expired (#113) dlm senarai sama.
> 117. `SQL_TAMBAHAN_140.sql` — **Tukar Produk Transaksi: pilih pekerja sumber stok + kuantiti** (susulan #115) — sebelum ni kuantiti dikunci ikut rekod asal & stok gantian WAJIB drpd pekerja yg sama (created_by transaksi), tak fleksibel utk kes cth produk betul cuma ada dlm bawaan pekerja LAIN, atau kuantiti sebenar pun tersalah rekod sekali. Modal "Tukar Produk" kini ada medan **Kuantiti** (boleh ubah, lalai ikut qty produk dipilih) & **Pekerja (Stok Diambil Dari)** (lalai pekerja asal transaksi, boleh tukar). `created_by` transaksi KEKAL tak berubah (upah/laporan kekal milik pekerja asal) — cuma sumber stok gantian boleh dipilih berlainan; produk LAMA sentiasa dipulangkan ke pekerja ASAL tak kira pekerja sasaran gantian dipilih siapa. Pengesahan tambahan: kuantiti baharu tak boleh kurang drpd jumlah dah disahkan terjual+pulang (consignment) — elak kuantiti "hilang" drpd rekod separa disahkan. Diuji live (BEGIN/ROLLBACK, S003 qty1→S001 qty2, pekerja sasaran ditukar) — semua kesan (stok bawaan 2 pekerja berbeza, created_by kekal, items/jumlah) betul terlaras.
> 118. `SQL_TAMBAHAN_141.sql` — **🚨 Baiki Tukar Produk Transaksi: pengesahan consignment lama terbawa, seksyen Follow-up hilang** (susulan #115/#117, ditemui pemilik sendiri guna live) — tukar_produk_transaksi() sebelum ni cuma "remap" stokId dlm items_terjual/items_pulang & kekalkan jualan_disahkan asal — munasabah utk qty/produk serupa, TAPI bila produk ditukar KEPADA produk lain sepenuhnya dgn qty jauh berbeza (kes sebenar: T5091574 1 unit Tamar Cocoa Papan → 20 unit sachet kecil), `jualan_disahkan=true` LAMA terbawa sekali → seksyen "🤝 Follow-up" (perlu `!jualan_disahkan`) TERUS TAK MUNCUL, walhal produk/kuantiti baharu tu belum pernah disahkan terjual/pulang. Baiki: transaksi consignment SENTIASA reset `jualan_disahkan=false` + `items_terjual`/`items_pulang=NULL` selepas tukar produk — paksa pengesahan SEGAR ikut produk/kuantiti baharu. Amaran turut ditambah pada dialog confirm() sblm tindakan (bukan cuma toast lepas siap). Diuji live (BEGIN/ROLLBACK) + betulkan T5091574 sedia ada yg terjejas terus di database — Follow-up (Terjual/Pulang) kini boleh diguna Aremier/pemilik sendiri utk sahkan 12 terjual + 8 pulang (baki auto-kredit balik ke bawaan pekerja bila disahkan — ciri sedia ada `sahkan_jualan_konsainan()`, disahkan berfungsi utk kuantiti > 1 & boleh diguna PEKERJA sendiri, bukan pemilik sahaja).
> 119. `SQL_TAMBAHAN_142.sql` + `SQL_TAMBAHAN_143.sql` — **📊 Storan Sistem (Live) + 🗄️ Arkib Bukti Bayaran Lama** (susulan siasatan storan Sep 2026, pelan Free 928MB/1GB storan fail = 91%) — 2 ciri baharu di Tetapan (pemilik sahaja):
> (a) **Storan Sistem (Live)** — RPC `get_storan_sistem()` pulangkan saiz database (Postgres) sebenar + pecahan saiz tiap bucket Supabase Storage. Kad baharu papar 2 bar-graf (Database & Storage) berwarna ikut had pelan Free (500MB/1GB — hijau <70%, kuning 70-90%, merah >90%) + carta bar kecil pecahan ikut bucket + amaran automatik bila hampir/cecah had. Shortcut badge diletak atas Dashboard (pemilik) — klik terus lompat ke kad ni.
> (b) **Arkib Bukti Bayaran Lama** — lajur baharu `tetapan.arkib_webhook_url` (editable dlm app) + edge function `arkib-bukti-bayaran` (RPC bantuan `get_saiz_fail_storan()`, akses ketat service_role sahaja). Butang "🔍 Semak" kira berapa fail bukti-bayaran (resit/transfer) yg rekod dah SELESAI (`transaksi.status='selesai'`) / DISAHKAN (`permohonan_bayaran_hutang.status='disahkan'`) & lebih 2 bulan; "🗄️ Arkibkan Sekarang" muat turun tiap fail, POST ke URL webhook luaran (Zapier/Make/n8n/server sendiri — pemilik pilih & tetapkan sendiri), PADAM drpd Supabase Storage bila berjaya dihantar, & kemaskini rujukan `resit_bukti_url` jadi `ARKIB:<url-baharu>` (jejak audit kekal, bukan hilang terus). Dihadkan 100 fail/panggilan (elak timeout) + tapis fail yg dah `ARKIB:` (elak diproses dua kali). Disahkan RPC get_storan_sistem() berfungsi live (928MB storan disahkan tepat); edge function disahkan deploy & gerbang JWT berfungsi (401 tanpa token) — 0 fail layak diarkibkan buat masa ini (sistem baru ~6 minggu, belum ada rekod >2 bulan).
> 120. **Kemaskini susulan #119**: (a) Edge function `arkib-bukti-bayaran` tukar payload webhook drpd `multipart/form-data` ke **JSON+base64** (`nama_fail`, `content_type`, `fail_base64`, `sumber`, `rekod_id`, `path_asal`) — mudahkan integrasi Microsoft Power Automate ("When a HTTP request is received" + "OneDrive - Create file" guna `base64ToBinary()`) tanpa perlu urai multipart yang rumit; (b) Syarat umur fail ditukar drpd **2 bulan → 1 bulan** (edge function + teks UI Tetapan); (c) **Akses Pantas Profile boleh disusun** — 2 shortcut baharu ("📊 Storan Sistem", "🗄️ Arkib Bukti Bayaran") ditambah ke `LEBIH_SHORTCUTS`, dan keseluruhan bar shortcut Profile kini ada butang "✏️ Susun" (anak panah ⬆️⬇️ pindah kedudukan, disimpan localStorage per-peranti) — pemilik/pekerja boleh susun ikut keutamaan sendiri, letak shortcut kerap guna di paling atas.
>
> 121. **🚨 Baiki "CPU Time exceeded" bila Arkibkan Bukti Bayaran** (ditemui pemilik guna sebenar — Power Automate webhook betul, punca sebenar edge function sendiri) — gelung manual `String.fromCharCode` per-byte utk tukar fail ke base64 terlajak had CPU Time edge function utk fail beberapa MB (biasa utk gambar resit), worker crash (status 546, mesej generik "Edge Function returned a non-2xx status code" di app). Baiki: guna `encodeBase64()` drpd `jsr:@std/encoding` (bukan gelung manual) — jauh lebih pantas & cekap CPU. Turut: (a) kurangkan had 100→20 fail/panggilan (lapisan keselamatan tambahan); (b) app kini cuba baca mesej ralat SEBENAR drpd badan respons (`error.context.json()`) bukan cuma mesej generik SDK; (c) papar jelas bila ada `baki_belum_diproses` (>20 fail layak) supaya pemilik tahu perlu tekan "Arkibkan Sekarang" beberapa kali utk proses semua.
> 122. **Tambah papar sebab kegagalan sebenar + log server** (susulan #121, ditemui pemilik: 0 berjaya, 20 gagal, tiada sebab dipaparkan) — app sebelum ni cuma tunjuk "N fail gagal (lihat log)" tanpa sebenarnya papar sebab, dan edge function tak `console.error()` langsung (jadi `function_logs` senyap juga). Baiki: (a) app papar terus senarai `data.ralat` (sebab kegagalan per-fail) dlm kad Arkib; (b) edge function kini `console.error()` setiap kegagalan (muat turun/webhook/padam storan/ralat lain) dgn mesej lengkap (termasuk baca badan respons webhook bila bukan 2xx) supaya boleh disemak dlm Supabase Logs bila perlu, bukan cuma bergantung pemilik screenshot.
> 123. **🚨 Baiki "request entity too large" pada Arkib Bukti Bayaran** (susulan #121/#122, ditemui pemilik sendiri semasa uji Make.com) — lepas isu OAuth Power Automate dibaiki (tukar ke Make.com), ujian sebenar dedah isu KEDUA: payload JSON+base64 (kandungan penuh gambar resit, few MB) ditolak webhook Make.com sbg "request entity too large" — had saiz webhook kebanyakan platform (Make/Zapier/Power Automate) jauh lebih kecil drpd saiz fail gambar sebenar. Baiki (reka bentuk baharu): edge function jana **SIGNED URL sementara** (sah 10 minit) drpd Supabase Storage & hantar PAUTAN sahaja dlm payload (nama_fail, file_url, sumber, rekod_id, path_asal) — BUKAN kandungan fail. Platform destinasi (Make.com "HTTP > Get a file", Power Automate "HTTP" action) muat turun terus drpd signed URL, baru upload ke OneDrive/Google Drive. Edge function jadi sangat ringan (tiada muat turun/encode dlm edge function terus) — elak SEPENUHNYA isu CPU Time (#121) DAN had saiz payload (#123) serentak. Had bilangan/panggilan turut dinaikkan 20→50 (edge function jauh lebih ringan sekarang).
> 124. **🚨 Baiki risiko KEHILANGAN FAIL pada Arkib Bukti Bayaran (kritikal)** (ditemui pemilik sendiri semasa uji Make.com — 20 fail resit transaksi Ogos 2026 didapati PADAM drpd Supabase Storage sebelum sempat sahkan sampai OneDrive) — punca: reka bentuk lama padam fail asal serta-merta lepas webhook pulangkan HTTP 200, tapi kebanyakan platform webhook (Make.com/Zapier) auto-balas 200 "Accepted" SERTA-MERTA bila permintaan diterima — BUKAN selepas keseluruhan aliran (termasuk upload OneDrive) selesai. Baiki (reka bentuk 2 LANGKAH): (1) "Hantar ke Webhook" — hantar signed URL sahaja, tanda rekod `ARKIB_MENUNGGU:<path>`, fail asal TIDAK disentuh; (2) butang berasingan "Sahkan & Padam Fail Asal" — HANYA lepas pemilik SENDIRI semak fail dah sampai OneDrive, baru fail asal dipadam. Ada juga butang "Batal" utk urungkan jika belum pasti. Kad Arkib di Tetapan kini papar notis kuning "menunggu pengesahan" secara automatik (termasuk di Dashboard) supaya tak terlupa sahkan/batal. **PENTING:** 20 fail resit (T2255866, T7777419, T0875658, T6449734, T1778915, T1693663, T1064079, T0472173, T0056443, T2874067, T0899852, T9458236, T8811500, T5607121, T4063858, T1106226, T8941868, T6121329, T9870726, T4568253) sudah hilang kekal drpd insiden ini — data transaksi (jumlah/produk) masih selamat, cuma gambar resit yang hilang.
