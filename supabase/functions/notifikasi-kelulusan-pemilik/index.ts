...
async function hantarTelegram(admin: any, jenis: string, pekerjaNama: string, butiran: string, recordId?: string, jenisRekod?: string): Promise<number> {
  ...
  if (recordId && jenisRekod && JADUAL_CODE[jenisRekod]) {
    ...
    const labelTolak = jenisRekod === "baucar_bayaran" ? "✕ Batal" : "✕ Tolak";
    replyMarkup = { inline_keyboard: [[
      { text: "✅ Lulus", callback_data: `tp:${code}:${recordId}:A` },
...
Deno.serve(async (req) => {
  ...
  try {
    const { jenis, pekerja_nama, butiran, record_id, jenis_rekod } = await req.json();
    if (!jenis) {
