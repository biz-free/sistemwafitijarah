...
async function cmdMenunggu(chatId: number) {
  const [{ data: cash }, { data: cuti }, { data: hutang }, { data: produk }, { data: padam }, { data: profiles }] = await Promise.all([
  ...
  const namaOf = (id: string) => profiles?.find((p: any) => p.id === id)?.nama || id;
  const total = (cash?.length || 0) + (cuti?.length || 0) + (hutang?.length || 0) + (produk?.length || 0) + (padam?.length || 0);
...
async function cmdStatus(chatId: number) {
  const [cashList, { count: hutangCount }, { data: tetapan }, { count: menungguCash }, { count: menungguCuti }, { count: menungguHutang }, { count: menungguProduk }] = await Promise.all([
  ...
  const totalMenunggu = (menungguCash || 0) + (menungguCuti || 0) + (menungguHutang || 0) + (menungguProduk || 0);
